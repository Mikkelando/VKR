...
def handle_phase(phase, *args, **kwargs):
    if phase == 'conf':
        ...
        maxiter = kwargs.get("maxiter", 100)  # Example max iterations
        sets = Sets(maxiter)
        print("Sets:")
        ...
        iterations = kwargs.get("iterations", [f"i{i}" for i in range(1, 101)])
        variables = kwargs.get("variables", ["MIU", "S"])
...
if __name__ == "__main__":
    # Example usage of handle_phase
    handle_phase('conf')
    handle_phase('sets', maxiter=100)
    handle_phase('gdx_items', iterations=[f"i{i}" for i in range(1, 101)], variables=["MIU", "S"])
